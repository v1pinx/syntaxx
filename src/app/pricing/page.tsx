...
export default function Pricing() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (data.status == 200) {
        const id = data.data.decoded.id;
        setUserId(id);
        const user = await axios.get(`/api/get-user?id=${id}`);
        if (user.status === 200) {
          setEmail(user.data.email);
          console.log("User data:", user.data);
        }
        if (user.data.data.isPro) {
          setIsPro(true);
  ...
  const handleSubscribe = async (planType: string) => {
    ...
    try {
      ...
      const orderData = await axios.post("/api/razorpay", orderBody, {
  ...
  const plans = [
    {
      ...
      price: "₹0",
      duration: "forever",
      features: ["Basic Code Editor", "Community support"],
  ...
  const features = [
    {
      ...
      description:
        "Support for over 3+ programming languages with specialized syntax highlighting and language tools.",
    },
    {
      title: "Integrated Terminal",
      description:
        "Run commands, scripts, and tests directly from within the editor environment.",
    },
  ];
      ...
      <div className=" flex flex-col py-16">
        ...
        <div className="text-center mb-12 z-10 animate-[fadeIn_0.8s_ease-in] px-4">
          ...
          <p className="text-[#6C7070] text-lg md:text-xl max-w-2xl mx-auto">
            Unlock powerful developer tools with our flexible plans. Code
            without limits.
          </p>
        </div>
        ...
        <div className="flex items-center justify-center mb-12 z-10 animate-[fadeIn_1s_ease-in]">
          <div className="flex p-1 bg-[#1A1D1D] rounded-lg border border-[#3E4343]">
            <button
          ...
          {plans.map((plan, index) => (
            ...
            >
              ...
              <div className="flex flex-col items-center md:items-start">
                ...
                <p className="text-[#6C7070] mb-6 text-center md:text-left">
                  {plan.description}
                </p>
              </div>
              ...
              {isPro && plan.name == "Pro" ? (
                <Button
                  className="w-full h-11 text-md rounded-2xl bg-gray-500 text-white cursor-not-allowed"
                  disabled
                  onClick={() => toast.error("Already Pro User!")}
                >
                  Already Subscribed
                </Button>
              ) : (
        ...
        <div className="mt-24 z-10 px-4 max-w-4xl mx-auto animate-[fadeIn_1.3s_ease-in]">
          <h2 className="text-2xl md:text-3xl text-white font-bold mb-8 text-center">
            Supported Languages & Frameworks
          </h2>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {["JavaScript", "TypeScript", "Python", "Java", "C++"].map(
              (lang, index) => (
                <div
                  key={index}
        ...
        <div className="mt-24 px-4 w-full max-w-4xl mx-auto animate-[fadeIn_1.5s_ease-in] z-10">
          <div className="p-8 md:p-12 bg-gradient-to-br from-[#1A1D1D] to-[#0E1313] rounded-2xl border border-[#3E4343] text-center">
            ...
            <p className="text-[#6C7070] mb-8 max-w-xl mx-auto">
              Experience the power of our code editor with a free account. No
